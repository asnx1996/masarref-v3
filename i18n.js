...
const I18N = {
  ...
  '🖱️ المؤشر والاختصارات (كمبيوتر بس)':'🖱️ Cursor & shortcuts (desktop only)',
  '✨ هالة المؤشر — ضوء يلحگ الماوس':'✨ Cursor glow — a light that follows the mouse',
  '🖱️ قائمة الكلك اليمين — اختصارات وتنقّل سريع':'🖱️ Right-click menu — shortcuts and quick navigation',
  'شنو يظهر بالقائمة؟':'What shows in the menu?',
...
/* ============================================================
   فهرس احتياطي: نفس المفاتيح بلا رموز
   ------------------------------------------------------------
   أغلب مفاتيح القاموس چانت تحمل الإيموجي وياها («🧾 المصاريف»)
   لأن الرمز چان جزء من النص. هسه الرموز الوظيفية صارت <svg>،
   يعني عقدة النص صارت «المصاريف» بلا رمز — والمفتاح القديم ما
   ينطابق، فالترجمة تسكت بصمت.

   بدل ما نعيد كتابة عشرات المفاتيح بالإيد (وننساها كل ما نبدّل
   رمز جديد)، نبني فهرس ثاني وقت التحميل: المفتاح بلا رموز →
   الترجمة بلا رموز. فالقديم والجديد يشتغلون سوا.
   ============================================================ */
/* الرموز اللي بأول النص أو بآخره بس — هذولي اللي چانوا أيقونات.
   ما نكشط رموز من وسط الجملة: «مصروف ← الصناديق» لازم يبقى مثل
   ما هو، والسهم بيه جزء من المعنى مو أيقونة زر. */
const I18N_EDGE = /^(?:[\u{1F300}-\u{1FAFF}\u{2190}-\u{2BFF}\u{FE0F}\u{FF0B}]+\s*)+|(?:\s*[\u{1F300}-\u{1FAFF}\u{2190}-\u{2BFF}\u{FE0F}\u{FF0B}]+)+$/gu;
function stripGlyphs(s){ return s.replace(I18N_EDGE, '').trim(); }
const I18N_BARE = (() => {
  const out = {};
  for(const k in I18N){
    const bk = stripGlyphs(k);
    /* أول مفتاح يفوز — ما ندوس على ترجمة موجودة أصلاً بلا رموز */
    if(bk && bk !== k && !I18N[bk] && !out[bk]) out[bk] = stripGlyphs(I18N[k]);
  }
  return out;
})();
...
function trString(s){
  ...
  if(I18N_BARE[s]) return I18N_BARE[s];
